...
class MemeticOptimizer():
    ...
    def optimize(self, population_size=10, block_size=32):
        ...
        self.initial_cpu_time = time.perf_counter() # CPU time tracking
        ...
        while self.check_continue(generationIdx):
            ...
            self.best_fitness_per_g.append(1 - max(fits))        # Save best fitness value
    ...
    def check_continue(self, generationIdx):
        if not self.undetected and generationIdx < self.maxGeneration: return True # Without premature convergence check
        # if not self.undetected and (generation < maxGeneration or (self.best_fitness_per_g[-10] - self.best_fitness_per_g[-1]) > 0.01): return True
        else: return False
    ...
    def fitness_individual(self, individual):
        ...
        print(pred, hashlib.md5(bytes(individual)).hexdigest(), hashlib.md5(open(self.binary, "rb").read()).hexdigest())
...
def getPredictionScore(filepath):
    score = float(runCommand(f"python3 wrapper-score.py {filepath} MalConv"))
    return score
